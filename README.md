...
> **Before publishing:** `paper/main.tex` (reproducibility section) and the citation block above still contain `TODO`/`REPLACE-ME` placeholders for the public repository URL, the archival DOI, and the venue. Push the repository first, then fill the URL, then mint a DOI by linking the repository to Zenodo and cutting a tagged release (Zenodo archives the release and issues a version DOI; cite that DOI for the submitted version).
